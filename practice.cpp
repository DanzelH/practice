// основной файл программы с телом "main"
#include <iostream>
#include <codecvt>
#include <ctime>
#include <iomanip>
		...
		if (!strcmp(data.Error.c_str(), "")) {
			...
			if (!sqlite3_open_v2(path.c_str(), &db, SQLITE_OPEN_READWRITE | SQLITE_OPEN_URI, NULL)) {
				...
				for (auto temp : data.myLogs) {
					...
					else fprintf(stderr, "ne mogu podgotovit' vyrajenie: %s\n", sqlite3_errmsg(db));
				}
				sqlite3_close(db);
			}
			else fprintf(stderr, "ne mogu otkrit' BD: %s\n", sqlite3_errmsg(db));
		}
	}
	else fprintf_s(stdout, "ne vernaya data");
	return 0;
}